...
export function DashboardShell({
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-gray-50">
      <Sidebar items={navItems} branding={branding} className="hidden md:flex" />
        ...
        <header className="z-30 flex h-16 shrink-0 items-center gap-2 border-b border-gray-200 bg-white/95 px-4 backdrop-blur-sm sm:px-6">
          ...
          {logoUrl.startsWith("data:") || logoUrl.startsWith("http") || logoUrl.startsWith("/") ? (
            ...
            <img
              src={logoUrl}
              alt={companyName}
              className="max-h-10 max-w-[200px] w-auto md:hidden object-contain object-left"
            />
          ) : (
            <span className="text-base font-bold tracking-tight text-brand md:hidden">
              {companyName}
            </span>
          )}
