    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">Live snapshot of your counter</p>
        </div>
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className={`mt-1 text-xs ${pendingActions > 0 ? "text-yellow-600" : "text-gray-500"}`}>
                {d.khatiPending} KYC · {d.redemptionsPending} redemptions
              </p>
            </div>
          ...
          {d.redemptionsPending > 0 && (
            <div className="flex items-center justify-between rounded-xl border border-orange-200 bg-orange-50 px-4 py-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-orange-800">{d.redemptionsPending} redemption{d.redemptionsPending > 1 ? "s" : ""} pending</p>
      ...
      {/* ── Two-column: Activity + Khati registrations ── */}
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-card">
          ...
          {d.recentKhatis.length === 0 ? (
            ...
            <ul className="divide-y divide-gray-50">
              {d.recentKhatis.map((k) => {
                const statusColor =
                  k.status === "active" ? "bg-green-50 text-green-700"
                  : k.kycStatus?.startsWith("pending") ? "bg-yellow-50 text-yellow-700"
                  : "bg-gray-100 text-gray-500";
                const statusLabel =
                  k.status === "active" ? "Active"
                  : k.kycStatus === "pending_counter" ? "Pending Review"
                  : k.kycStatus === "pending_sales_rep" ? "Pending Sales"
                  : k.kycStatus === "pending_admin" ? "Pending Admin"
                  : k.status;
                return (
                  <li key={k.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50/60">
                    ...
                    <div className="text-right">
                      ...
                      <p className="mt-0.5 text-[10px] text-gray-400">{timeAgo(k.createdAt)}</p>
                    </div>
                  </li>
                );
              })}
            ...
            <div className="px-4 py-3">
              <table className="min-w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {d.topKhatis.map((k, i) => {
                    const max = d.topKhatis[0].points || 1;
                    const total = d.pointsDistributed || 1;
                    const pct = Math.round((k.points / total) * 100);
                    return (
                      <tr key={i} className="hover:bg-gray-50/40">
                        ...
                        <td className="py-2 text-right font-bold text-brand-dark">{k.points.toLocaleString()}</td>
